  ...
  methods: {
    ...
    async initMarker(){
      ...
      for (var i = 0, marker; i < this.tree.length; i++) { // 因为我需要展示多个 所以必须使用循环
        ...
        let lnglat = [this.tree[i].geo_property.longitude, this.tree[i].geo_property.latitude]
        ...
        marker.content = `<div style="width: 460px">
                           ...
                           <p>经纬度：${lnglat[0]}&#176，${lnglat[1]}&#176</p>
                           <p>地址：${this.tree[i].place_name}</p>
                           <p>管护单位：${this.tree[i].gh_unit}</p>
                           </div>
                           <img src='${this.getPicApi}${this.tree[i].pic_record.path[0]}' style="width: 200px"/>
                           </div>
