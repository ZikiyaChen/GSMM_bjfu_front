    ...
    <Table border stripe :columns="columns" :data="Measuresdata"  size="small"  >
      ...
      <template slot-scope="{ row, index }" slot="action">
        <div v-if="editIndex === index">
          <Button @click="handleSave(row,index)" type="primary" size="small" style="margin-right: 10px">保存</Button>
        ...
        <div v-else>
          ...
          <Button @click="handleDelete(row, index)" type="error" size="small" style="margin-left: 10px">删除</Button>
        </div>
...
<!--    <div style="display: table-cell;;width: 500px;height: auto"></div>-->
<!--    <div style="display: table-cell;;width:500px;height: auto"></div>-->
<!--  </div>-->
...
<script>
import {
  AddMeasures, AddMethods,
  deleteYhMeasure, deleteYhMethod,
  queryMeasures,
  queryMethods,
  updateYhMeasure,
  updateYhMethod
...
export default {
  ...
  methods:{
    ...
    fetchMeasures(){
      queryMeasures({'yh_type':'修剪'}).then((measures=>{
        this.Measuresdata = measures.data.yh_measures
      }))
    },
    fetchMethods(){
      queryMethods({'yh_type':'修剪'}).then((res=>{
