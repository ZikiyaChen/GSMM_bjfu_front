<template>
  <div>
    <Card>
      <h2 slot="title" style="text-align: center">生长环境评价分析---修改</h2>
...
import {
  evaluationList,
  habitat_typeList, has_structuresList,
  highlandList, is_buriedList,
  is_pollutionList, organic_contentList, other_plantsList,
  plainList,
  soil_textureList,
  variaList
...
export default {
  ...
  methods: {
    ...
    fetchEnvironmentDate(){
      getNewGeAnalysis(this.tree_code).then((res=>{
        this.environment = res.data.new_ge_analysis
        console.log(this.environment.id)
        getOneTjxmRecord(this.environment.id,'生长环境评价分析').then((resp=>{
          console.log('@@@',resp.data)
          this.tjxm_record = resp.data.record
          this.environment.investigate_username = resp.data.record.username
          console.log(this.environment.investigate_username)
          this.fetchPicData()
        }))
      }))
